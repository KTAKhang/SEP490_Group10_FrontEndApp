...
export async function getOrderByUserApi(page = 1, limit = 10, options = {}) {
  try {
    ...
    if (status_names && String(status_names).trim() !== "") {
      const normalized = String(status_names)
        .trim()
        .toUpperCase()
        .replace(/-/g, "_");
    ...
    const response = await axios.get(
      `${API_BASE_URL}/order/my-orders?${params.toString()}`,
      {
        headers: {
    ...
    if (firstOrder) {
      const hasDetails =
        Array.isArray(firstOrder.details) && firstOrder.details.length > 0;
      console.log(
        "[OrderList API] first order has details?",
        hasDetails,
        "details?.length=",
        firstOrder.details?.length ?? 0,
      );
    }
...
export async function getMyOrderByIdApi(orderId) {
  try {
    ...
    const result = response.data;
    console.log(
      "[OrderDetail API] response.data keys:",
      result ? Object.keys(result) : "null",
    );
    console.log(
      "[OrderDetail API] result.status:",
      result?.status,
      "result.data?",
      !!result?.data,
    );
    if (result?.data) {
      console.log(
        "[OrderDetail API] result.data keys:",
        Object.keys(result.data),
      );
      console.log(
        "[OrderDetail API] result.data.details length:",
        result.data.details?.length ?? "no details",
      );
    }
    ...
    else if (orderObj && Array.isArray(orderObj.details))
      detailsArr = orderObj.details;
    else if (orderObj && Array.isArray(orderObj.order_details))
      detailsArr = orderObj.order_details;

    console.log(
      "[OrderDetail API] parsed details length:",
      detailsArr.length,
      "orderObj?",
      !!orderObj,
    );
    ...
    console.log(
      "[OrderDetail API] returning details.length:",
      out.details?.length ?? 0,
    );
    return out;
  } catch (error) {
    console.log(
      "[OrderDetail API] ERROR",
      error?.message,
...
export async function returnOrderApi(order_id) {
  try {
    ...
    throw new Error(
      error.response?.data?.message || error.message || "Không thể trả hàng",
    );
  }
}
